...
export type RdoFoto = Tables<"rdo_fotos">;
...
/**
 * Busca a obra + todos os RDOs com seus filhos (fotos, pendências,
 * pontos de atenção, equipe NUE, terceiros) em uma única roundtrip
 * usando joins do PostgREST.
 */
export async function fetchDiarioObra(id: string): Promise<DiarioObraData> {
  const [obraRes, rdosRes] = await Promise.all([
    supabase
      .from("obras")
      .select(
  ...
  const rdos = (rdosRes.data ?? []).map((r): RdoCompleto => ({
    ...
    fotos: ((r as { fotos: RdoFoto[] }).fotos ?? []).slice().sort(
      (a, b) => a.ordem - b.ordem,
    ),
    pendencias: ((r as { pendencias: RdoPendencia[] }).pendencias ?? []).slice().sort(
      (a, b) => a.ordem - b.ordem,
    ),
    pontos_atencao: ((r as { pontos_atencao: RdoPontoAtencao[] }).pontos_atencao ?? []).slice().sort(
      (a, b) => a.ordem - b.ordem,
    ),
    equipe_nue: ((r as { equipe_nue: RdoEquipeNue[] }).equipe_nue ?? []).slice().sort(
      (a, b) => a.ordem - b.ordem,
    ),
    terceiros: ((r as { terceiros: RdoTerceiro[] }).terceiros ?? []).slice().sort(
      (a, b) => a.ordem - b.ordem,
    ),
  }));
